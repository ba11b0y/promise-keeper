...
class SupabaseManager: ObservableObject {
    ...
    private init() {
        ...
        NSLog("🔍 Initializing Supabase with URL: %@", supabaseURL)
        NSLog("🔍 Supabase key prefix: %@...", String(supabaseKey.prefix(20)))
        
        // Validate URL
        guard let url = URL(string: supabaseURL), url.host != nil else {
    ...
    func signUp(email: String, password: String) async {
        ...
        do {
            ...
            NSLog("❌ Full error: %@", String(describing: error))
            
            if let urlError = error as? URLError {
                NSLog("❌ URLError code: %@", urlError.code.rawValue.description)
                NSLog("❌ URLError failing URL: %@", urlError.failingURL?.absoluteString ?? "nil")
                switch urlError.code {
                case .cannotFindHost:
                    errorMessage = "Cannot connect to authentication server (\(urlError.failingURL?.host ?? "unknown host")). Please check your internet connection and try again."
    ...
    private func handleAuthStateChange(event: AuthChangeEvent, session: Session?) {
        ...
        case .signedIn:
            if let user = session?.user, let session = session {
                ...
                TokenRefreshManager.shared.startTokenRefreshTimer()
            }
        case .signedOut:
            ...
            TokenRefreshManager.shared.stopTokenRefreshTimer()
