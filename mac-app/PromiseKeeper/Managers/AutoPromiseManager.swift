...
class AutoPromiseManager: ObservableObject {
        ...
        do {
            ...
            if !response.promises.isEmpty {
                ...
                if !createdPromises.isEmpty {
                    ...
                    // Refresh the promise list to ensure UI is updated
            ...
            if let bamlError = error as? BAMLError {
                ...
                case .httpError(let code):
                    ...
                    if code == 401 {
                        processingStatus = "Please sign in to process screenshots"
                    }
    ...
    private func showPromiseCreatedNotification(count: Int, screenshotId: String) async {
        // Send individual notifications for each promise with formatted content
        for (index, promise) in recentlyCreatedPromises.prefix(3).enumerated() {
            let formattedTitle = promise.formatted?.title ?? "New Promise"
            let formattedBody = promise.formatted?.body ?? promise.content
            let formattedDetails = promise.formatted?.details
            
            notificationManager.sendNotification(
                title: formattedTitle,
                body: formattedBody,
                subtitle: formattedDetails,
                timeInterval: Double(index) * 0.5 // Stagger notifications
            )
        }
        
        // If more than 3 promises, send a summary notification
        if count > 3 {
            notificationManager.sendNotification(
                title: "Promise Keeper",
                body: "Found \(count) promises total",
                timeInterval: 2.0
            )
        }
    }
